...
# The script is in scripts/lms/, so project root is two levels up
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../" && pwd)"
cd "$PROJECT_ROOT"

# Check if the distribution zip exists
DIST_ZIP="modules/lms/service/target/lms-service-1.0-SNAPSHOT-dist.zip"
if [ ! -f "$DIST_ZIP" ]; then
    echo "Error: Distribution artifact not found at $DIST_ZIP"
    echo "Please run ./scripts/lms/build.sh first."
    exit 1
fi
