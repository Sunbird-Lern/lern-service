...
# The script is in scripts/userorg/, so project root is two levels up
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../../" && pwd)"
cd "$PROJECT_ROOT"

# Check if the distribution zip exists
DIST_ZIP="modules/userorg/controller/target/userorg-service-1.0-SNAPSHOT-dist.zip"
if [ ! -f "$DIST_ZIP" ]; then
    echo "Error: Distribution artifact not found at $DIST_ZIP"
    echo "Please run ./scripts/userorg/build.sh first."
    exit 1
fi

echo "Building Docker image..."
docker build -f build/userorg/Dockerfile -t "$FULL_IMAGE_NAME" --build-arg CSP="${CSP}" .
