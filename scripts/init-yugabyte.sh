#!/usr/bin/env bash
# Downloads CQL migration scripts from sunbird-spark-installer and applies them
# against the local YugabyteDB container.
#
# Usage: ./scripts/init-yugabyte.sh [ENVIRONMENT] [BRANCH]
#   ENVIRONMENT: keyspace prefix (default: dev)
#   BRANCH:      branch of sunbird-spark-installer to use (default: develop)
#
# Prerequisites: Docker must be running with the yugabyte container healthy.
