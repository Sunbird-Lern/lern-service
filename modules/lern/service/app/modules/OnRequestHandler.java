...
public class OnRequestHandler implements ActionCreator {
        ...
        return new Action.Simple() {
            ...
            public CompletionStage<Result> call(Http.Request request) {
                ...
                if (ConfigFactory.load().getBoolean(JsonKey.AUTH_ENABLED)) {
                    message = (String) util.LernServiceRequestInterceptor.verifyRequestData(request, new java.util.HashMap<>()).get(JsonKey.USER_ID);
                } else {
    ...
    private String getRequestedForId(Http.Request request) {
        ...
        if(null != jsonBody && jsonBody.has(JsonKey.REQUEST) && jsonBody.get(JsonKey.REQUEST).has(JsonKey.USER_ID)) {
            ...
            // Logic for read APIs where ID is in URL
            Path path = Paths.get(request.uri());
            String uuidSegment = path.getFileName().toString();
             try {
                  // Basic check if last segment is UUID
                  UUID.fromString(uuidSegment);
                  requestedForUserID = uuidSegment;
    ...
    private Http.Request intializeRequestInfo(Http.Request request, String userId, String requestId) { 
        try {
            ...
            if (StringUtils.isBlank(requestId)) {
                requestId = JsonKey.DEFAULT_CONSUMER_ID; // Fallback
            }
