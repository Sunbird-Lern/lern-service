...
/**
 * Canonical viewer request keys — the viewer contract is collectionId / contextId / contentId only.
...
object ViewerRequestKeys {
  ...
  def collectionId(request: Request): Option[String] = value(request, "collectionId")
  def contextId(request: Request): Option[String] = value(request, "contextId")
