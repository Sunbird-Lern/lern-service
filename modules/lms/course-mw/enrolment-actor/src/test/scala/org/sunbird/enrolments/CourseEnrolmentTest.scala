...
class CourseEnrolmentTest extends FlatSpec with Matchers with MockFactory {
    ...
    "CourseEnrolmentActor" should "return success on enrol" in {
        ...
        val response = callActor(getEnrolRequest(), Props(new TestCourseEnrolmentActor(cacheUtil).setDao(courseDao, userDao, groupDao)))
    ...
    "On invalid course batch" should "return client error" in  {
        ...
        (userDao.read(_: RequestContext, _: String,_: String,_: String)).expects(*,*,*,*).returns(null)
        val response = callActorForFailure(getEnrolRequest(), Props(new TestCourseEnrolmentActor(cacheUtil).setDao(courseDao, userDao, groupDao)))
    ...
    "Unenrol" should "return success on enrol" in {
        ...
        (userDao.updateV2(_: RequestContext, _: String,_: String,_: String, _: java.util.Map[String, AnyRef])).expects(*,*,*,*,*)
        (cacheUtil.delete(_: String)).expects(*).once()
        val response = callActor(getUnEnrolRequest(), Props(new TestCourseEnrolmentActor(cacheUtil).setDao(courseDao, userDao, groupDao)))
        assert("Success".equalsIgnoreCase(response.get("response").asInstanceOf[String]))
    }
    ...
    "already inactive" should " return client error on unenroll" in {
        ...
        (userDao.read(_: RequestContext, _: String,_: String,_: String)).expects(*,*,*,*).returns(userCourse)
        val response = callActorForFailure(getUnEnrolRequest(), Props(new TestCourseEnrolmentActor(cacheUtil).setDao(courseDao, userDao, groupDao)))
        assert(response.getErrorResponseCode == ResponseCode.CLIENT_ERROR.getResponseCode)
    }
    ...
    "listEnrol" should "return success on listing" in {
        ...
        (userDao.listEnrolments(_: RequestContext, _: String, _: java.util.List[String])).expects(*,*,*).returns(getEnrolmentLists())
            val response = callActor(getListEnrolRequest(), Props(new TestCourseEnrolmentActor(cacheUtil).setDao(courseDao, userDao, groupDao)))
        println(response)
...
//        assert(null == courses.get(2).get("lastContentAccessTime"))
    ...
    "listEnrol with RedisConnector is true" should "return success on listing from redis RedisConnector" in {
        ...
        val response = callActor(request, Props(new TestCourseEnrolmentActor(cacheUtil).setDao(courseDao, userDao, groupDao)))
        println(response)
        assert(null != response)
    }
...
class TestCourseEnrolmentActor(cacheUtil: RedisCacheUtil) extends CourseEnrolmentActor(null)(cacheUtil) {
  ...
  override def addCourseDetails(activeEnrolments: java.util.List[java.util.Map[String, AnyRef]], courseIds: java.util.List[String], request: Request): java.util.List[java.util.Map[String, AnyRef]] = {
    activeEnrolments
  }

  override def addBatchDetails(enrolmentList: java.util.List[java.util.Map[String, AnyRef]], request: Request): java.util.List[java.util.Map[String, AnyRef]] = {
    enrolmentList
  }
}
