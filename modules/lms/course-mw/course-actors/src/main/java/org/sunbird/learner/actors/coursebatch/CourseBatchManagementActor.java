...
public class CourseBatchManagementActor extends BaseActor {
  ...
  // Context key carrying the top-level (root) batch id down the auto-create recursion, so every
  // chained descendant batch is one level under the root (`rootBatch:courseId`), never deep-chained.
  ...
  //  updateBatchCount(courseBatch);
    ...
    // A trackable collection whose trackablenodes contain other trackable collections auto-creates a
    // batch for each trackable child by re-triggering this same actor; every child batch is one level
    // under the root batch (`rootBatch:courseId`). A collection with no trackablenodes ⇒ no-op.
    // Best-effort — the batch response is already sent above.
  ...
  /**
   * Fan out child-batch creation for a trackable collection. Reads {@code trackablenodes} for
   * {@code parentCourseId}; for each trackable child, re-sends a {@code createBatch} to this actor
   * with a flat root-prefixed batchId ({@code rootBatchId:childCourseId}). The root batch id is
   * threaded through unchanged so deeper nesting stays one level (never {@code root:parent:child}).
   * The existence guard makes it idempotent, so a child already reached via the flat trackablenodes
   * list is not re-created when its parent recurses.
   */
  private void triggerChildBatchCreation(Request parent, String parentCourseId, String rootBatchId) {
    ...
    try {
      for (String childCourseId : getTrackableNodes(parentCourseId, ctx)) {
        if (StringUtils.equalsIgnoreCase(childCourseId, parentCourseId)) continue;  // guard: self-reference
        String childBatchId = rootBatchId + ":" + childCourseId;
        if (batchExists(childCourseId, childBatchId, ctx)) continue;                 // idempotent: safe re-run / gap-fill
        Request child = new Request(ctx);
        child.setOperation("createBatch");
        child.getContext().putAll(parent.getContext());
        child.getContext().put(ROOT_BATCH_ID, rootBatchId);                          // keep root prefix through recursion
  ...
  /** Reads the ordered trackable course ids from the `hierarchy_relations` row emitted at publish. */
