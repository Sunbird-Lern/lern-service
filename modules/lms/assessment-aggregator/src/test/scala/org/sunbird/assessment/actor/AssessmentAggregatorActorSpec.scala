...
import org.sunbird.assessment.service.{CassandraService, ContentMetadata, ContentService, KafkaService}
...
class AssessmentAggregatorActorSpec extends TestKit(ActorSystem("AssessmentAggregatorActorSpec"))
  ...
  def getActorRef = {
    TestActorRef(new AssessmentAggregatorActor(Some(mCassandra), Some(mKafka), Some(mContent)))
  }
  ...
  it should "process a single assessment request with events" in {
    reset(mContent, mCassandra, mKafka)
    val existing = ExistingAssessment("a1", "cont1", System.currentTimeMillis(), System.currentTimeMillis(), 10.0, 10.0, List.empty)
    when(mContent.fetchMetadata(anyString, any[RequestContext])).thenReturn(ContentMetadata(isValid = true, totalQuestions = 10))
  ...
  it should "return error response when userId is missing" in {
    ...
    response.getErrorResponseCode should be (400)
  }
  ...
  it should "skip stale assessment" in {
    ...
    org.mockito.Mockito.doReturn(ContentMetadata(isValid = true, totalQuestions = 10))
      .when(mContent).fetchMetadata(org.mockito.ArgumentMatchers.anyString, org.mockito.ArgumentMatchers.any(classOf[RequestContext]))
  ...
  it should "throw exception when content validation fails" in {
    ...
    actorRef ! request
    val response = expectMsgType[ProjectCommonException]
    response.getErrorResponseCode should be (ResponseCode.CLIENT_ERROR.getResponseCode)
  ...
  it should "handle missing config for skipMissing (default to true)" in {
    reset(mContent, mCassandra, mKafka)
    when(mContent.fetchMetadata(any[String], any[RequestContext])).thenReturn(ContentMetadata(isValid = true, totalQuestions = 1))
