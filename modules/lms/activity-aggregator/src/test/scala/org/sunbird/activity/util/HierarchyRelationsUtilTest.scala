...
/**
 * HierarchyRelationsUtil: the trackablenodes reader and the JVM-wide TTL cache on readFromDB.
...
class HierarchyRelationsUtilTest extends AnyFlatSpec with Matchers with MockFactory {
  ...
  // --- getTrackableNodes: reads <root>:<root>:trackablenodes, ordered, no dedup ---
