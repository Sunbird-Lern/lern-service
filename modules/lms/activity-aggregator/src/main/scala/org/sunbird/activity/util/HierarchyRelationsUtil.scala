...
class HierarchyRelationsUtil(cassandraOperation: CassandraOperation) {
  ...
  /**
   * Ordered trackable course ids for an LP root, from the `<root>:<root>:trackablenodes` relation
   * (emitted at publish). Order is preserved (unlock order); NOT de-duplicated. Empty if absent.
   */
