...
public class ResetPasswordService {
  ...
  public String getSunbirdLoginUrl() {
    String webUrl = ProjectUtil.getConfigValue(JsonKey.SUNBIRD_WEB_URL);
    String loginPageUrl = ProjectUtil.getConfigValue(JsonKey.PASSWORD_RESET_LOGIN_PAGE_URL);
    if (StringUtils.isNotBlank(webUrl) && webUrl.endsWith("/") && StringUtils.isNotBlank(loginPageUrl) && loginPageUrl.startsWith("/")) {
        return webUrl + loginPageUrl.substring(1);
    }
    return webUrl + loginPageUrl;
  }
}
